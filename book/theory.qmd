---
title: "Theory: Compound Poisson–Gamma, Tweedie Pure Premium, and Exposure Weights"
---
...
For $1<p<2$, the Tweedie pure-premium distribution follows exactly from a compound Poisson model with Gamma severities.
...
The annualized claim-frequency rate $\nu_i$ stays the same. Exposure converts it into the expected count over the observed risk period.

Unit exposure means $e_i=1$.
...
For the exact Tweedie compound-Poisson representation we assume Gamma severities, which have strictly positive support.
...
## Aggregate claim cost: the compound Poisson model
...
## Variance of aggregate claim cost
...
## The exact Tweedie connection

For $1<p<2$, the Tweedie distribution is exactly a compound Poisson distribution with Gamma jumps.

- Delong, Lindholm and Wüthrich [1, Sections 2.1-2.2, Proposition 2.1] derive this result by comparing moment-generating functions. They start from a compound Poisson–Gamma model with claim count mean equal to exposure times claim frequency and identify the corresponding Tweedie parameters.
- Jørgensen and de Souza [2] is a foundational actuarial reference for the same compound-Poisson Tweedie process.

## Mean-dispersion notation for the Tweedie distribution
...
This notation separates the **observation-specific dispersion** $d_i$ from the common base parameter $\phi$.

## Mapping Tweedie parameters to the compound Poisson–Gamma model
...
## Recovering frequency times severity
...
## Explicit derivation of the Tweedie variance rate
...
## Introducing arbitrary exposure
...
Both mean and variance scale linearly with risk exposure.

## From aggregate loss to observed pure premium
...
Under the compound Poisson–Gamma assumptions, $R_i$ follows an exact Tweedie distribution.

## Exact distribution of the pure premium

### Demonstration by scaling the compound Poisson representation
...
### Demonstration by matching the Tweedie compound-Poisson parameters
...
This distribution follows from:
...
## The aggregate loss is Tweedie too, but with a different dispersion
...
## Why the pure-premium Tweedie uses exposure as its weight {#sec-tweedie-exposure-weight}
...
## The same result from the Tweedie likelihood
...
## The same result from Tweedie deviance
...
## Why this is not the same as forcing common dispersion on aggregate loss {#sec-common-dispersion}
...
## What a single Tweedie model assumes about frequency and severity

A single Tweedie pure-premium model with fixed $p$ and common base dispersion $\phi$ links frequency and severity.
...
Under the compound Poisson–Gamma assumptions, the quantities relate as follows:

| Quantity | Model or identity |
| --- | --- |
| Claim count | $N_i\mid X_i,e_i\sim\operatorname{Poisson}(e_i\nu_i)$ |
| Claim severity | $Z_{ij}\mid X_i\sim\operatorname{Gamma}(\gamma,c_i)$, with mean $\zeta_i=\gamma/c_i$ |
| Aggregate cost | $S_i=\sum_{j=1}^{N_i}Z_{ij}$, with conditional mean $e_i\nu_i\zeta_i=e_i\mu_i$ |
| Pure-premium rate | $R_i=S_i/e_i$, with conditional mean $\mu_i=\nu_i\zeta_i$ |
| Tweedie representation | $R_i\mid X_i,e_i\sim\operatorname{Tw}_p(\mu_i,\phi/e_i)$ for $1<p<2$ |

The conditional variance is $\phi\mu_i^p/e_i$, giving `sample_weight = exposure`
for the rate model.

## Minimal practical implementations
...
## Assumptions behind exposure weighting
...
## Poisson counts with exposure offset versus claim-frequency rates with exposure weight {#sec-poisson-offset-weight}
...
### Formulation 1: claim count with a log-exposure offset
...
### Formulation 2: observed frequency with exposure weight
...
### Equality of the score equations
...
### Exact Poisson deviance identity
...
### Why Poisson is especially simple
...
## Compact notation map to Delong, Lindholm and Wüthrich
